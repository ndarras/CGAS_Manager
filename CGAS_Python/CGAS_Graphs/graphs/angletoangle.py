...
class atoa:
    ...
    def plotatoa(self):
            ...
            for i in range(self.plotlevels):
                for j in range(self.planes):
                    if self.plotlevels > 1 and self.planes > 1:
                        ...
                        ax = axs[j]
                    plevs = self.plotlevels - 1
                    ax.set_ylabel(self.plot_names[i][0], labelpad=0.2, fontsize=6)
                    ax.set_xlabel(self.plot_names[plevs-j][0], labelpad=0.2, fontsize=6)
                    ...
                    ax.linewidth = 0.50
                    try:
                        ax.fill_between(self.ref_data[g][comp-j][0], self.refsdminus_data[g][i][0], self.refsdplus_data[g][i][0], color=self.group_colours[g], alpha=0.2)
                        ax.plot(self.ref_data[g][comp-j][0], self.ref_data[g][i][0], color=self.group_colours[g], linewidth=1, alpha=0.80)
                        ax.scatter(self.ref_data[g][comp - j][0][0], self.ref_data[g][i][0][0], color=self.group_colours[g],
                ...
                for j in range(self.planes):
                    ...
                    if len(self.division_lines[g][0]) > 0:
                         ...
                         try:
                             ...
                             ax.plot(self.y_data[g][comp-j][0], self.y_data[g][i][0], c=self.line_colours[g][0], linewidth=0.5,
                                      picker=True)
                         except IndexError:
        ...
        fig.canvas.manager.set_window_title(self.graph_type + "_Angle_to_Angle")
        if self.groupno > 0  and self.gno == 0:
            plt.savefig(self.graph_type + "_Angle_to_Angle_AVG" + ".png")
        else:
            plt.savefig(self.graph_type + "_Angle_to_Angle" + ".png")
