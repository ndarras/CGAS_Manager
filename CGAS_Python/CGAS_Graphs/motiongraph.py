...
class GaitGraph:
    def __init__(self, graph_type, graph_name):
            ...
            for i in range(self.plotlevels):
                for j in range(self.planes):
                    if self.plotlevels > 1 and self.planes > 1:
                        ...
                        ax = axs[j]
                        ax.set_title(self.plot_names[i][j], fontsize=8)
                        text = self.self.plot_axis_xyz[i][j].split("|")
                        ax.set_ylabel(text[1], labelpad=None, fontsize=6)
                        ax.tick_params(axis='x', labelsize=6)
                        ax.tick_params(axis='y', labelsize=6)
                    else:
                        ax.set_visible(False)
                    ...
                    if len(self.division_lines[g][0]) > 0:
                        ...
                        try:
                            ax.plot(self.x_data[g][i], self.y_data[g][i][j], c=self.line_colours[g][0], linewidth=0.5, picker=True)
                        except IndexError:
    ...
    if event.key == 'ctrl+shift':
        ...
        for zoom_axis in event.canvas.figure.axes:
            ...
            if zoom_axis.get_title() != '':
                ...
                logging.info("save to:" + "_" + global_graph_type + "_" + "{:0>2}".format(i) + "_" + zoom_axis.get_title())
...
def print_selected_line(lineno):
    tkinter.messagebox.showinfo("Information", "selected line: " + str(lineno+1))
    logging.info("selected line: " + str(lineno+1))
